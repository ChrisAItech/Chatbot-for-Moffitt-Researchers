...
def process_dataframe_for_display(records):
    ...
    Ensures proper type handling for Arrow serialization.
    ...
    for record in records:
        ...
        # Function to format score fields with explicit float conversion
        def format_score(score):
            if score is None or score == 'N/A' or score == 'UNKNOWN':
                return None  # Using None instead of 'N/A' for better type handling
            try:
                return float(score)
        ...
        record_summary = {
            ...
            'File Name': str(record.get('pdf_file_name', '')),
            'Organ': str(record.get('organ', '')),
            'Site': str(stage_3_data.get('site', '')),
            'Stage': str(stage_3_data.get('stage', '')),
            'Laterality': str(stage_3_data.get('laterality', '')),
            'Grade': str(stage_3_data.get('grade', '')),
    ...
    # Convert score columns to float type with explicit NA handling
    score_columns = ['Site Score', 'Stage Score', 'Laterality Score', 'Grade Score']
    for col in score_columns:
        df[col] = pd.to_numeric(df[col], errors='coerce')
    
    # Convert string columns to string type, replacing empty strings with None
    string_columns = ['Record ID', 'File Name', 'Organ', 'Site', 'Stage', 'Laterality', 'Grade']
...
def display_database_records(records):
    """
    Displays an overview of all records stored in MongoDB with proper handling of NA values.
    ...
    # Format the display of NA values
